...
*/
  ...
  public static void DestroyTag(string _tag)
  {
    ...
    GetUnityTags().Where(t => t.Split(MultiTagsUtil.TAG_SEPARATOR).Contains(_tag)).ToList().ForEach(t => DestroyUnityTag(t));
  ...
  {
    if (!AssetDatabase.LoadAssetAtPath<TagsSO>(MultiTagsUtil.TAGS_ASSET_PATH)) CreateTagsAsset();
  }
  ...
  {
    TagsSO _tagsSO = AssetDatabase.LoadAssetAtPath<TagsSO>(MultiTagsUtil.TAGS_ASSET_PATH);
  ...
  public static TagsSO CreateTagsAsset()
  {
    ...
    Directory.CreateDirectory(Application.dataPath.Replace("Assets", "") + Path.GetDirectoryName(MultiTagsUtil.TAGS_ASSET_PATH).Replace("\\", "/"));
    AssetDatabase.CreateAsset(_reference, MultiTagsUtil.TAGS_ASSET_PATH);
    AssetDatabase.SaveAssets();
  ...
  {
    return GetUnityTags().SelectMany(t => t.Split(MultiTagsUtil.TAG_SEPARATOR)).Distinct().ToArray();
  }
  ...
  public static void AddTagToGameObjects(string _tagName, GameObject[] _gameObjects)
  {
    ...
    foreach (GameObject _gameObject in _gameObjects)
    {
      if (!_gameObject.GetTagNames().Contains(_tagName))
      {
        ...
        if (_gameObject.tag == MultiTagsUtil.BuiltInTagsNames[0])
        {
        ...
        {
          string[] _newTags = (_gameObject.tag + MultiTagsUtil.TAG_SEPARATOR + _tagName).Split(MultiTagsUtil.TAG_SEPARATOR);
          Array.Sort(_newTags, StringComparer.InvariantCulture);
          string _newTag = _newTags[0];

          for (int _i = 1; _i < _newTags.Length; _i++)
          {
            _newTag += MultiTagsUtil.TAG_SEPARATOR + _newTags[_i];
          }
  ...
  public static void RemoveTagFromGameObjects(string _tagName, GameObject[] _gameObjects)
  {
    ...
    string[] _previousTags = _gameObjects.Where(g => g.tag.Contains(MultiTagsUtil.TAG_SEPARATOR)).Select(g => g.tag).Distinct().ToArray();
    ...
    foreach (GameObject _object in _gameObjects)
    {
      ...
      {
        if (_object.tag.Contains(MultiTagsUtil.TAG_SEPARATOR + _tagName + MultiTagsUtil.TAG_SEPARATOR))
        {
          _newTag = _object.tag.Replace(MultiTagsUtil.TAG_SEPARATOR + _tagName + MultiTagsUtil.TAG_SEPARATOR, MultiTagsUtil.TAG_SEPARATOR.ToString());
        }
        else if (_object.tag.Substring(0, _tagName.Length + 1) == _tagName + MultiTagsUtil.TAG_SEPARATOR)
        {
      ...
      {
        _newTag = MultiTagsUtil.BuiltInTagsNames[0];
      }
  ...
  {
    string[] _customUnityTags = GetUnityTags().Except(MultiTagsUtil.BuiltInTagsNames).ToArray();
  ...
  private static Rect GUITagsField(Rect _position, Tag[] _tags, Action<Tag> _addTagCallback, Action<Tag> _removeTagCallback, bool _doIgnoreUntaggedTag)
  {
    ...
    if (_canAddTag)
    {
      ...
      if (_doIgnoreUntaggedTag) _tagsToAdd = _tagsToAdd.Where(t => t.Name != MultiTagsUtil.BuiltInTagsNames[0]).ToArray();
    ...
    for (int _i = 0; _i < _tags.Length; _i++)
    {
      ...
      if (_doIgnoreUntaggedTag && (_tag.Name == MultiTagsUtil.BuiltInTagsNames[0])) continue;
