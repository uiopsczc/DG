...
namespace DG
{
	public class ZipUtil
	{
		...
		public static bool CreateZipFile(string directoryPath, string zipFilePath)
		{
			...
			{
				DGLog.Error(string.Format("Cannot find directory '{0}'", directoryPath));
				return false;
			}
			...
			{
				isSuccess = false;
				DGLog.Error(string.Format("Exception during processing {0}", ex));
			}
		...
		public void UnZipFile(string zipFilePath)
		{
			...
			{
				DGLog.Error(string.Format("Cannot find file '{0}'", zipFilePath));
				return;
			}
