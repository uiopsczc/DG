        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            using (new EditorGUIBeginPropertyScope(position, label, property))
            {
                ...
                using (new EditorGUIIndentLevelScope(-EditorGUI.indentLevel))
                {
                    ...
                    var typeRect = new Rect(typeLableRect.xMax + splitWidth, position.y, typeNameRectWidth,
                        position.height);
                    var valueLableRect = new Rect(typeRect.xMax + splitWidth, position.y, valueLableRectWidth,
                        position.height);
                    var valueRect = new Rect(valueLableRect.xMax + splitWidth, position.y, valueRectWidth,
                        position.height);


                    var valueParseTypeList =
                        ValueParseUtil.GetValueParseList().ConvertAll(e => ((Type)e["type"])).ToList();
                    var valueParseTypeFullNames = valueParseTypeList.ConvertAll(e => e.FullName).ToArray();
                    var valueParseTypeDisplayNames =
                        valueParseTypeList.ConvertAll(e => e.Name.ToGUIContent()).ToArray();
