...
namespace DG
{
    ...
    {
        private const int _BYTE_WIDTH = 32; //uint是32位
        ...
        {
            _bytes = new uint[(size - 1) / _BYTE_WIDTH + 1];
        }
        ...
        {
            var stringBuilder = new StringBuilder(_bytes.Length * _BYTE_WIDTH);
            for (var i = _bytes.Length - 1; i >= 0; i--)
                stringBuilder.Append(Convert.ToString(_bytes[i], 2).PadLeft(_BYTE_WIDTH, '0'));
