...
namespace DG
{
    public class FPBoundingBox
    {
        ...
        {
            return set(
                min.set(FPMath.Min(min.x, a_bounds.min.x), FPMath.Min(min.y, a_bounds.min.y),
                    FPMath.Min(min.z, a_bounds.min.z)),
                max.set(FPMath.Max(max.x, a_bounds.max.x), FPMath.Max(max.y, a_bounds.max.y),
                    FPMath.Max(max.z, a_bounds.max.z)));
        }
        ...
        {
            return set(
                min.set(FPMath.Min(min.x, center.x - radius), FPMath.Min(min.y, center.y - radius),
                    FPMath.Min(min.z, center.z - radius)),
                max.set(FPMath.Max(max.x, center.x + radius), FPMath.Max(max.y, center.y + radius),
                    FPMath.Max(max.z, center.z + radius)));
        }
