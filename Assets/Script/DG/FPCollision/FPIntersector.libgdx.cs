...
namespace DG
{
    public class FPIntersector
    {
        ...
        public static int pointLineSide(FP linePoint1X, FP linePoint1Y, FP linePoint2X, FP linePoint2Y, FP pointX,
            FP pointY)
        {
            return FPMath.Sign((linePoint2X - linePoint1X) * (pointY - linePoint1Y) -
        ...
        public static bool intersectPolygons(FPPolygon p1, FPPolygon p2, FPPolygon overlap = null)
        {
            ...
            FPVector2 ip = FPIntersector.ip,
                ep1 = FPIntersector.ep1,
                ep2 = FPIntersector.ep2,
                s = FPIntersector.s,
                e = FPIntersector.e;
        ...
        public static FPVector2 nearestSegmentPoint(FPVector2 start, FPVector2 end, FPVector2 point,
         ...
         * @return Whether the line segment and the circle intersect */
        public static bool intersectSegmentCircle(FPVector2 start, FPVector2 end, FPCircle circle,
        ...
        public static bool intersectFrustumBounds(FPFrustum frustum, FPBoundingBox bounds)
        {
            bool boundsIntersectsFrustum = frustum.pointInFrustum(bounds.getCorner000(ref tmp))
                                           || frustum.pointInFrustum(bounds.getCorner001(ref tmp)) ||
                                           frustum.pointInFrustum(bounds.getCorner010(ref tmp))
                                           || frustum.pointInFrustum(bounds.getCorner011(ref tmp)) ||
                                           frustum.pointInFrustum(bounds.getCorner100(ref tmp))
                                           || frustum.pointInFrustum(bounds.getCorner101(ref tmp)) ||
                                           frustum.pointInFrustum(bounds.getCorner110(ref tmp))
        ...
        {
            return intersectBoundsPlaneFast(box.getCenter(ref tmp1), box.getDimensions(ref tmp2).scl(0.5f),
         ...
         * @return Whether the bounding box and the plane intersect. */
        public static bool intersectBoundsPlaneFast(FPVector3 center, FPVector3 halfDimensions, FPVector3 normal,
            FP distance)
        {
            ...
            ref FPRectangle intersection)
        {
            if (rectangle1.overlaps(rectangle2))
            {
                if (!intersection.Equals(FPRectangle.Null))
                {
                    ...
                    intersection.width = FPMath.Min(rectangle1.x + rectangle1.width, rectangle2.x + rectangle2.width) -
                                         intersection.x;
                    intersection.y = FPMath.Max(rectangle1.y, rectangle2.y);
                    intersection.height =
                        FPMath.Min(rectangle1.y + rectangle1.height, rectangle2.y + rectangle2.height) - intersection.y;
                }
        ...
        public static bool intersectSegmentRectangle(FP startX, FP startY, FP endX, FP endY, FPRectangle rectangle)
        {
            ...
            if (intersectSegments(startX, startY, endX, endY, rectangle.x, rectangle.y, rectangleEndX, rectangle.y))
                return true;
        ...
        public static bool overlapConvexPolygons(FP[] verts1, FP[] verts2)
        {
            return overlapConvexPolygons(verts1, 0, verts1.Length, verts2, 0, verts2.Length,
                ref FPMinimumTranslationVector.Null);
        }
         ...
         * @param mtv A Minimum Translation Vector to fill in the case of a collision, or null (optional).
         * @return Whether polygons overlap. */
        public static bool overlapConvexPolygons(FP[] verts1, int offset1, int count1, FP[] verts2, int offset2,
            int count2,
            ref FPMinimumTranslationVector mtv)
        {
            bool overlaps;
        ...
        {
            return overlapConvexPolygons(verts1, offset1, count1, verts2, offset2, count2,
                ref FPMinimumTranslationVector.Null);
        }
        ...
        private static bool overlapsOnAxisOfShape(FP[] verts1, int offset1, int count1, FP[] verts2, int offset2,
            int count2,
        ...
        public static void splitTriangle(FP[] triangle, FPPlane plane, FPSplitTriangle split)
        {
            ...
            bool r2 = plane.testPoint(triangle[0 + stride], triangle[1 + stride], triangle[2 + stride]) ==
                      DGPlaneSide.Back;
