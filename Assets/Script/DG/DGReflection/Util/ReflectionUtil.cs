...
namespace DG
{
	public partial class ReflectionUtil
	{
		...
		{
			MethodInfo result = GetMethodInfoCache2(type, methodName);
			if (result!= null)
				return result;
		...
		{
			var result = GetMethodInfoCache(type, methodName, sourceParameterTypes);
			if (result != null)
				return result;
			...
			string dllName = null, params object[] sourceParameters)
		{
			...
			return type == null
				? null
				: GetMethodInfo(type, methodName, bindingFlags, getMethodInfosFunc, sourceParameters);
		}
		...
		{
			PropertyInfo result = GetPropertyInfoCache(type, propertyName);
			if (result != null)
				return result;
		...
		public static FieldInfo GetFieldInfo(Type type, string fieldName,
			BindingFlags bindingFlags = BindingFlagsConst.All)
		{
			FieldInfo result = GetFieldInfoCache(type, fieldName);
			if (result != null)
				return result;
