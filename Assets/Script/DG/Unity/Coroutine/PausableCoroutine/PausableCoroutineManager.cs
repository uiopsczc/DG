...
namespace DG
{
    ...
    {
        private Dictionary<string, List<PausableCoroutine>> _name2PausableCoroutineList = new();

        private List<List<PausableCoroutine>> _tempCoroutineList = new();

        private readonly Dictionary<string, Dictionary<string, PausableCoroutine>> _ownerHash2PausableCoroutineDict =
            new();
        ...
        {
            if (!_name2PausableCoroutineList.ContainsKey(routine.routineUniqueHash)) return;
            _ownerHash2PausableCoroutineDict[routine.ownerUniqueHash].Remove(routine.routineUniqueHash);
            _name2PausableCoroutineList.Remove(routine.routineUniqueHash);
        }

        void GoStopAllCoroutines(object thisReference)
        {
            PausableCoroutine coroutine = CreateCoroutine(null, thisReference);
            if (!_ownerHash2PausableCoroutineDict.TryGetValue(coroutine.ownerUniqueHash, out var value)) return;
            foreach (var couple in value)
                _name2PausableCoroutineList.Remove(couple.Value.routineUniqueHash);

            _ownerHash2PausableCoroutineDict.Remove(coroutine.ownerUniqueHash);
        }
        ...
        {
            if (!_name2PausableCoroutineList.ContainsKey(coroutine.routineUniqueHash))
            {
                List<PausableCoroutine> newCoroutineList = new List<PausableCoroutine>();
                _name2PausableCoroutineList.Add(coroutine.routineUniqueHash, newCoroutineList);
            }

            _name2PausableCoroutineList[coroutine.routineUniqueHash].Add(coroutine);

            if (!_ownerHash2PausableCoroutineDict.ContainsKey(coroutine.ownerUniqueHash))
            {
                Dictionary<string, PausableCoroutine> newCoroutineDict = new Dictionary<string, PausableCoroutine>();
                _ownerHash2PausableCoroutineDict.Add(coroutine.ownerUniqueHash, newCoroutineDict);
            }
            ...
            if (!_ownerHash2PausableCoroutineDict[coroutine.ownerUniqueHash].ContainsKey(coroutine.routineUniqueHash))
            {
                _ownerHash2PausableCoroutineDict[coroutine.ownerUniqueHash].Add(coroutine.routineUniqueHash, coroutine);
            }
        ...
        void Update()
        {
            ...
            if (deltaTime == 0f)
                return;
            if (_name2PausableCoroutineList.Count == 0)
            {
                return;
            }

            _tempCoroutineList.Clear();
            foreach (var pair in _name2PausableCoroutineList)
            ...
            for (var i = _tempCoroutineList.Count - 1; i >= 0; i--)
            {
                ...
                for (int j = coroutines.Count - 1; j >= 0; j--)
                {
                    ...
                    {
                        _name2PausableCoroutineList.Remove(coroutine.ownerUniqueHash);
                    }
                }
            }
        }
