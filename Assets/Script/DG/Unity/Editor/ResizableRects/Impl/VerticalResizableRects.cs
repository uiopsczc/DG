...
namespace DG
{
    ...
    {
        public VerticalResizableRects(Func<Rect> total_rect_func, float[] split_pixels, float[] splitPcTs = null) :
            base(
        ...
        protected override void UpdateSplitLineRects()
        {
            for (int i = 0; i < splitLineRects.Length; i++)
            {
                ...
                else
                {
                    ...
                    splitLineRects[i].Set(total_rect.x, total_rect.y + split_pct * total_rect.height, total_rect.width,
                        1);
                }
            }
        }
