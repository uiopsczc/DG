	...
	public partial struct DGMatrix4x4
	{
		...
		*************************************************************************************/
		...
		public static DGMatrix4x4 CreatePerspectiveFieldOfViewRad(DGFixedPoint fieldOfViewRadian, DGFixedPoint aspectRatio, DGFixedPoint nearClip, DGFixedPoint farClip)
		{
			...
			var fd = (DGFixedPoint)1.0 / DGFixedPointMath.Tan(fieldOfViewRadian / (DGFixedPoint)2.0);
		...
		*************************************************************************************/
			...
			DGVector3 cameraUpVector, DGVector3 cameraForwardVector)
		{
			...
			if (norm < DGFixedPointMath.Epsilon)
				zaxis = -cameraForwardVector;
			else
				zaxis = zaxis * (DGFixedPoint)1.0f / DGFixedPointMath.Sqrt(norm);
		...
		{
			DGFixedPoint minAngle = (DGFixedPoint)1.0f - ((DGFixedPoint)0.1f * (DGFixedPointMath.PI / (DGFixedPoint)180.0f)); // 0.1 degrees
			...
			if (norm < DGFixedPointMath.Epsilon)
				...
				faceDir = faceDir * ((DGFixedPoint)1.0f / DGFixedPointMath.Sqrt(norm));
			...
			if (DGFixedPointMath.Abs(dot) > minAngle)
			{
				...
				if (DGFixedPointMath.Abs(dot) > minAngle)
				{
					zaxis = (DGFixedPointMath.Abs(rotateAxis.z) > minAngle) ? new DGVector3(1, 0, 0) : new DGVector3(0, 0, -1);
				}
