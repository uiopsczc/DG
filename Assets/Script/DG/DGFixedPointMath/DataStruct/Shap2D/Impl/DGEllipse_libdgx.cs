...
namespace DG
{
	public struct DGEllipse : IDGShape2D
	{
		...
		{
			return DGFixedPointMath.PI * (this.width * this.height) / (DGFixedPoint)4;
		}
		...
		public DGFixedPoint circumference()
		{
			...
			if (a * (DGFixedPoint)3 > b || b * (DGFixedPoint)3 > a)
			{
				...
				return DGFixedPointMath.PI * (((DGFixedPoint)3 * (a + b)) - DGFixedPointMath.Sqrt(((DGFixedPoint)3 * a + b) * (a + (DGFixedPoint)3 * b)));
			}

			// We can use the simpler approximation, then
			return DGFixedPointMath.TwoPI * DGFixedPointMath.Sqrt((a * a + b * b) / (DGFixedPoint)2);
		}
