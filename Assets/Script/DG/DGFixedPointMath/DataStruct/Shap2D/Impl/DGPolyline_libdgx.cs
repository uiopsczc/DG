...
namespace DG
{
	public class DGPolyline : IDGShape2D
	{
		...
		public DGFixedPoint[] getTransformedVertices()
		{
			...
			DGFixedPoint cos = DGFixedPointMath.CosDeg(rotation);
			DGFixedPoint sin = DGFixedPointMath.SinDeg(rotation);
		...
		public DGFixedPoint getLength()
		{
			...
			for (int i = 0, n = localVertices.Length - 2; i < n; i += 2)
			{
				...
				length += DGFixedPointMath.Sqrt(x * x + y * y);
			}
		...
		public DGFixedPoint getScaledLength()
		{
			...
			for (int i = 0, n = localVertices.Length - 2; i < n; i += 2)
			{
				...
				scaledLength += DGFixedPointMath.Sqrt(x * x + y * y);
			}
