...
namespace DG
{
	public struct DGRectangle : IDGShape2D
	{
		...
		{
			DGFixedPoint minX = DGFixedPointMath.Min(x, rect.x);
			DGFixedPoint maxX = DGFixedPointMath.Max(x + width, rect.x + rect.width);
			x = minX;
			width = maxX - minX;

			DGFixedPoint minY = DGFixedPointMath.Min(y, rect.y);
			DGFixedPoint maxY = DGFixedPointMath.Max(y + height, rect.y + rect.height);
			y = minY;
			height = maxY - minY;
		...
		{
			DGFixedPoint minX = DGFixedPointMath.Min(this.x, x);
			DGFixedPoint maxX = DGFixedPointMath.Max(this.x + width, x);
			this.x = minX;
			this.width = maxX - minX;

			DGFixedPoint minY = DGFixedPointMath.Min(this.y, y);
			DGFixedPoint maxY = DGFixedPointMath.Max(this.y + height, y);
			this.y = minY;
		...
		public DGRectangle merge(DGVector2[] vecs)
		{
			...
			{
				DGVector2 v = vecs[i];
				minX = DGFixedPointMath.Min(minX, v.x);
				maxX = DGFixedPointMath.Max(maxX, v.x);
				minY = DGFixedPointMath.Min(minY, v.y);
				maxY = DGFixedPointMath.Max(maxY, v.y);
			}
