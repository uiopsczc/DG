...
namespace DG
{
	...
	public partial struct DGQuaternion
	{
		...
		public DGVector3 eulerAngles
		{
			...
			set => this = Internal_FromEulerRad(value * DGFixedPointMath.Rad2Deg);
		}
		...
		*************************************************************************************/
		...
		{
			return DGFixedPointMath.IsApproximatelyZero((DGFixedPoint)1 - q.w * q.w - q.x * q.x - q.y * q.y - q.z * q.z);
		}
		...
		public static DGVector3 ToYawPitchRoll(DGQuaternion value, DGVector3 vector)
		{
			...
			var z = DGFixedPointMath.Atan2(sinrCosp, cosrCosp);
			// pitch (y-axis rotation)
			var sinp = (DGFixedPoint)2 * (value.w * value.x - value.y * value.z);
			DGVector3 result = DGVector3.zero;
			if (DGFixedPointMath.Abs(sinp) >= DGFixedPoint.One)
				result.y = DGFixedPointMath.CopySign(DGFixedPointMath.HalfPi, sinp);
			else
				result.y = DGFixedPointMath.Asin(sinp);
			// yaw (z-axis rotation)
			...
			result.x = DGFixedPointMath.Atan2(sinYcosp, cosYcosp);
			return result;
		}

		public static DGQuaternion Normalize(DGQuaternion q)
		{
			DGFixedPoint num = DGFixedPointMath.Sqrt(Dot(q, q));
			if (num < DGFixedPointMath.Epsilon)
				return identity;
		...
		{
			DGFixedPoint dot = DGFixedPointMath.Min(DGFixedPointMath.Abs(Dot(a, b)), (DGFixedPoint)1);
			return _IsEqualUsingDot(dot) ? DGFixedPoint.Zero : DGFixedPointMath.Acos(dot) * (DGFixedPoint)2 * DGFixedPointMath.Rad2Deg;
		}
		...
		{
			DGFixedPoint negativeFlip = (DGFixedPoint)(-0.0001f) * DGFixedPointMath.Rad2Deg;
			...
			DGFixedPoint maxDegreesDelta)
		{
			...
			if (angle == DGFixedPoint.Zero)
				return to;
			return SlerpUnclamped(from, to, DGFixedPointMath.Min((DGFixedPoint)1, maxDegreesDelta / angle));
		}

		public static DGQuaternion CreateFromAxisAngle(DGVector3 axis, DGFixedPoint angle)
		{
			return CreateFromAxisAngleRad(axis, angle * DGFixedPointMath.Deg2Rad);
		}

		public static DGQuaternion CreateFromAxisAngleRad(DGVector3 axis, DGFixedPoint radians)
		{
			var theta = radians * DGFixedPointMath.Half;
			var sin = DGFixedPointMath.Sin(theta);
			var cos = DGFixedPointMath.Cos(theta);
			var x = axis.x * sin;
			var y = axis.y * sin;
			var z = axis.z * sin;
			var w = cos;
		...
		private static DGQuaternion INTERNAL_CALL_AngleAxis(DGFixedPoint degress, ref DGVector3 axis)
		{
			...
			var radians = degress * DGFixedPointMath.Deg2Rad;
			radians *= (DGFixedPoint)0.5f;
			axis.Normalize();
			axis = axis * DGFixedPointMath.Sin(radians);
			result.x = axis.x;
			result.y = axis.y;
			result.z = axis.z;
			result.w = DGFixedPointMath.Cos(radians);
		...
		private static DGQuaternion INTERNAL_CALL_LookRotation(ref DGVector3 forward, ref DGVector3 up)
		{
			...
			{
				var num = DGFixedPointMath.Sqrt(num8 + (DGFixedPoint)1f);
			...
			if ((m00 >= m11) && (m00 >= m22))
			{
				var num7 = DGFixedPointMath.Sqrt((((DGFixedPoint)1f + m00) - m11) - m22);
			...
			if (m11 > m22)
			{
				var num6 = DGFixedPointMath.Sqrt((((DGFixedPoint)1f + m11) - m00) - m22);
			...
			var num5 = DGFixedPointMath.Sqrt((((DGFixedPoint)1f + m22) - m00) - m11);
		...
		{
			var hr = roll * DGFixedPointMath.Half;
			var hp = pitch * DGFixedPointMath.Half;
			var hy = yaw * DGFixedPointMath.Half;
			var shr = DGFixedPointMath.Sin(hr);
			var chr = DGFixedPointMath.Cos(hr);
			var shp = DGFixedPointMath.Sin(hp);
			var chp = DGFixedPointMath.Cos(hp);
			var shy = DGFixedPointMath.Sin(hy);
			var chy = DGFixedPointMath.Cos(hy);
		...
		public static DGFixedPoint GetAxisAngleRadFromQuaternion(DGQuaternion q, out DGVector3 axis)
		{
			...
			{
				axis = axis / DGFixedPointMath.Sqrt(lengthSquared);
				radian = (DGFixedPoint)2 * DGFixedPointMath.Acos(DGFixedPointMath.Clamp(qw, (DGFixedPoint)(-1), DGFixedPoint.One));
			}
			else
			{
				axis = DGVector3.up;
		...
		public static DGFixedPoint GetAxisAngleFromQuaternion(DGQuaternion q, out DGVector3 axis)
		{
			...
			return radian * DGFixedPointMath.Rad2Deg;
		}
		...
		{
			pct = DGFixedPointMath.Clamp01(pct);
		...
		public static DGQuaternion SlerpUnclamped(DGQuaternion start, DGQuaternion end, DGFixedPoint pct)
		{
			...
			{
				var angle = DGFixedPointMath.Acos(dot);

				var invSinAngle = (DGFixedPoint)1 / DGFixedPointMath.Sin(angle);

				var t1 = DGFixedPointMath.Sin(((DGFixedPoint)1 - pct) * angle) * invSinAngle;

				var t2 = DGFixedPointMath.Sin(pct * angle) * invSinAngle;
		...
		{
			return DGFixedPointMath.Sqrt(SqrMagnitude(value));
		}
		...
		{
			return Internal_FromEulerRad(v * DGFixedPointMath.Deg2Rad);
		}
		...
		private static DGVector3 Internal_ToEulerRad(DGQuaternion rotation)
		{
			...
			if (test > (DGFixedPoint)0.4995f * unit)
			{
				...
				v.y = (DGFixedPoint)2f * DGFixedPointMath.Atan2(rotation.y, rotation.x);
				v.x = DGFixedPointMath.HalfPi;
				v.z = (DGFixedPoint)0;
				return NormalizeAngles(v * DGFixedPointMath.Rad2Deg);
			}
			...
			v.y = DGFixedPointMath.Atan2((DGFixedPoint)2f * q.x * q.w + (DGFixedPoint)2f * q.y * q.z,
				(DGFixedPoint)1 - (DGFixedPoint)2f * (q.z * q.z + q.w * q.w)); // Yaw
			v.x = DGFixedPointMath.Asin((DGFixedPoint)2f * (q.x * q.z - q.w * q.y)); // Pitch
			v.z = DGFixedPointMath.Atan2((DGFixedPoint)2f * q.x * q.y + (DGFixedPoint)2f * q.z * q.w,
		...
		private static DGQuaternion Internal_FromEulerRad(DGVector3 euler)
		{
			...
			DGFixedPoint sinRollOver2 = DGFixedPointMath.Sin(rollOver2);
			DGFixedPoint cosRollOver2 = DGFixedPointMath.Cos(rollOver2);
			DGFixedPoint pitchOver2 = pitch * (DGFixedPoint)0.5f;
			DGFixedPoint sinPitchOver2 = DGFixedPointMath.Sin(pitchOver2);
			DGFixedPoint cosPitchOver2 = DGFixedPointMath.Cos(pitchOver2);
			DGFixedPoint yawOver2 = yaw * (DGFixedPoint)0.5f;
			DGFixedPoint sinYawOver2 = DGFixedPointMath.Sin(yawOver2);
			DGFixedPoint cosYawOver2 = DGFixedPointMath.Cos(yawOver2);
			DGQuaternion result;
		...
		{
			if (DGFixedPointMath.Abs(q.w) > (DGFixedPoint)1.0f)
				q.Normalize();


			angle = (DGFixedPoint)2.0f * DGFixedPointMath.Acos(q.w); // angle
			DGFixedPoint den = DGFixedPointMath.Sqrt((DGFixedPoint)1.0 - q.w * q.w);
		...
		*************************************************************************************/
		...
		public void ToAngleAxis(out DGFixedPoint angle, out DGVector3 axis)
		{
			...
			angle *= DGFixedPointMath.Rad2Deg;
		}
