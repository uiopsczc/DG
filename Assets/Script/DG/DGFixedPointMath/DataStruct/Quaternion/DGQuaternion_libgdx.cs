...
namespace DG
{
	...
	public partial struct DGQuaternion
	{
		...
		public DGQuaternion(DGVector3 axis, DGFixedPoint angle)
		{
			...
			DGFixedPoint radians = angle * DGFixedPointMath.Deg2Rad;
			...
			{
				d = (DGFixedPoint)1f / d;
				DGFixedPoint l_ang = radians < (DGFixedPoint)0 ? DGFixedPointMath.TwoPI - (-radians % DGFixedPointMath.TwoPI) : radians % DGFixedPointMath.TwoPI;
				DGFixedPoint l_sin = DGFixedPointMath.Sin(l_ang / (DGFixedPoint)2);
				DGFixedPoint l_cos = DGFixedPointMath.Cos(l_ang / (DGFixedPoint)2);
		...
		{
			return DGFixedPointMath.Sqrt(x * x + y * y + z * z + w * w);
		}
		...
		{
			return setEulerAnglesRad(yaw * DGFixedPointMath.Deg2Rad, pitch * DGFixedPointMath.Deg2Rad,
				roll * DGFixedPointMath.Deg2Rad);
		}
		...
		public DGQuaternion setEulerAnglesRad(DGFixedPoint yaw, DGFixedPoint pitch, DGFixedPoint roll)
		{
			...
			DGFixedPoint shr = DGFixedPointMath.Sin(hr);
			DGFixedPoint chr = DGFixedPointMath.Cos(hr);
			DGFixedPoint hp = pitch * (DGFixedPoint)0.5f;
			DGFixedPoint shp = DGFixedPointMath.Sin(hp);
			DGFixedPoint chp = DGFixedPointMath.Cos(hp);
			DGFixedPoint hy = yaw * (DGFixedPoint)0.5f;
			DGFixedPoint shy = DGFixedPointMath.Sin(hy);
			DGFixedPoint chy = DGFixedPointMath.Cos(hy);
		...
		public DGFixedPoint getRollRad()
		{
			...
			return pole == (DGFixedPoint)0
				? DGFixedPointMath.Atan2((DGFixedPoint)2f * (w * z + y * x), (DGFixedPoint)1f - (DGFixedPoint)2f * (x * x + z * z))
				: pole * (DGFixedPoint)2f * DGFixedPointMath.Atan2(y, w);
		}
		...
		{
			return getRollRad() * DGFixedPointMath.Rad2Deg;
		}
		...
		public DGFixedPoint getPitchRad()
		{
			DGFixedPoint pole = getGimbalPole();
			return pole == (DGFixedPoint)0
				? DGFixedPointMath.Asin(DGFixedPointMath.Clamp((DGFixedPoint)2f * (w * x - z * y), (DGFixedPoint)(-1f), (DGFixedPoint)1f))
				: pole * DGFixedPointMath.PI * (DGFixedPoint)0.5f;
		}
		...
		{
			return getPitchRad() * DGFixedPointMath.Rad2Deg;
		}
		...
		public DGFixedPoint getYawRad()
		{
			return getGimbalPole() == (DGFixedPoint)0
				? DGFixedPointMath.Atan2((DGFixedPoint)2f * (y * w + x * z), (DGFixedPoint)1f - (DGFixedPoint)2f * (y * y + x * x))
				: (DGFixedPoint)0f;
		}
		...
		{
			return getYawRad() * DGFixedPointMath.Rad2Deg;
		}
		...
		public DGQuaternion nor()
		{
			...
			if (len != (DGFixedPoint)0f && !DGFixedPointMath.IsEqual(len, (DGFixedPoint)1f))
			{
				len = DGFixedPointMath.Sqrt(len);
				w /= len;
				x /= len;
				y /= len;
				z /= len;
		...
		{
			return DGFixedPointMath.IsZero(x) && DGFixedPointMath.IsZero(y) && DGFixedPointMath.IsZero(z) && DGFixedPointMath.IsEqual(w, (DGFixedPoint)1f);
		}

		/** @return If this quaternion is an identity Quaternion */
		public bool isIdentity(DGFixedPoint tolerance)
		{
			return DGFixedPointMath.IsZero(x, tolerance) && DGFixedPointMath.IsZero(y, tolerance) && DGFixedPointMath.IsZero(z, tolerance)
				   && DGFixedPointMath.IsEqual(w, (DGFixedPoint)1f, tolerance);
		}
		...
		{
			return setFromAxisRad(x, y, z, degrees * DGFixedPointMath.Deg2Rad);
		}
			...
			DGFixedPoint zy, DGFixedPoint zz)
		{
			...
			{
				// |w| >= .5
				DGFixedPoint s = DGFixedPointMath.Sqrt(t + (DGFixedPoint)1); // |s|>=1 ...
			...
			{
				DGFixedPoint s = DGFixedPointMath.Sqrt((DGFixedPoint)1.0 + xx - yy - zz); // |s|>=1
			...
			{
				DGFixedPoint s = DGFixedPointMath.Sqrt((DGFixedPoint)1.0 + yy - xx - zz); // |s|>=1
			...
			{
				DGFixedPoint s = DGFixedPointMath.Sqrt((DGFixedPoint)1.0 + zz - xx - yy); // |s|>=1
		...
		{
			DGFixedPoint dot = DGFixedPointMath.Clamp(v1.dot(v2), (DGFixedPoint)(-1f), (DGFixedPoint)1f);
			DGFixedPoint angle = DGFixedPointMath.Acos(dot);
		...
		public DGQuaternion setFromCross(DGFixedPoint x1, DGFixedPoint y1, DGFixedPoint z1, DGFixedPoint x2, DGFixedPoint y2,
			DGFixedPoint z2)
		{
			DGFixedPoint dot = DGFixedPointMath.Clamp(DGVector3.dot(x1, y1, z1, x2, y2, z2), (DGFixedPoint)(-1f), (DGFixedPoint)1f);
			DGFixedPoint angle = DGFixedPointMath.Acos(dot);
		...
		public DGQuaternion slerp(DGQuaternion end, DGFixedPoint alpha)
		{
			...
			if (((DGFixedPoint)1 - absDot) > (DGFixedPoint)0.1)
			{
				...
				DGFixedPoint angle = DGFixedPointMath.Acos(absDot);
				DGFixedPoint invSinTheta = (DGFixedPoint)1f / DGFixedPointMath.Sin(angle);

				// Calculate the scale for q1 and q2, according to the angle and
				// it's sine value
				scale0 = (DGFixedPointMath.Sin(((DGFixedPoint)1f - alpha) * angle) * invSinTheta);
				scale1 = (DGFixedPointMath.Sin((alpha * angle)) * invSinTheta);
			}
		...
		public DGQuaternion exp(DGFixedPoint alpha)
		{
			...
			DGFixedPoint normExp = DGFixedPointMath.Pow(norm, alpha);

			// Calculate theta
			DGFixedPoint theta = DGFixedPointMath.Acos(w / norm);

			// Calculate coefficient of basis elements
			DGFixedPoint coeff = (DGFixedPoint)0;
			if (DGFixedPointMath.Abs(theta) < (DGFixedPoint)0.001
				) // If theta is small enough, use the limit of sin(alpha*theta) / sin(theta) instead of actual
				  // value
				coeff = normExp * alpha / norm;
			else
				coeff = normExp * DGFixedPointMath.Sin(alpha * theta) / (norm * DGFixedPointMath.Sin(theta));

			// Write results
			w = normExp * DGFixedPointMath.Cos(alpha * theta);
			x *= coeff;
			y *= coeff;
			z *= coeff;
		...
		{
			return getAxisAngleRad(out axis) * DGFixedPointMath.Rad2Deg;
		}
		...
		public DGFixedPoint getAxisAngleRad(out DGVector3 axis)
		{
			...
			DGFixedPoint angle = (DGFixedPoint)2.0 * DGFixedPointMath.Acos(this.w);
			DGFixedPoint s = DGFixedPointMath.Sqrt((DGFixedPoint)1 - this.w *
							   this.w); // assuming quaternion normalised then w is less than 1, so term always positive.
			if (s < DGFixedPointMath.Epsilon)
			{
		...
		{
			return (DGFixedPoint)2.0 * DGFixedPointMath.Acos((this.w > (DGFixedPoint)1) ? (this.w / len()) : this.w);
		}
		...
		{
			return getAngleRad() * DGFixedPointMath.Rad2Deg;
		}
		...
		public DGFixedPoint getAngleAroundRad(DGFixedPoint axisX, DGFixedPoint axisY, DGFixedPoint axisZ)
		{
			...
			return DGFixedPointMath.IsZero(l2)
				? (DGFixedPoint)0f
				: (DGFixedPoint)2.0 * DGFixedPointMath.Acos(DGFixedPointMath.Clamp((d < (DGFixedPoint)0 ? -this.w : this.w) / DGFixedPointMath.Sqrt(l2), -(DGFixedPoint)1f,
					  (DGFixedPoint)1f));
		}
		...
		{
			return getAngleAroundRad(axisX, axisY, axisZ) * DGFixedPointMath.Rad2Deg;
		}
