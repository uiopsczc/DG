...
namespace DG
{
	...
	public class DelaunayTriangulator
	{
		...
		{
			DGFixedPoint xc, yc;
			DGFixedPoint y1y2 = DGFixedPointMath.Abs(y1 - y2);
			DGFixedPoint y2y3 = DGFixedPointMath.Abs(y2 - y3);
			if (y1y2 < DGFixedPointMath.Epsilon)
			{
				if (y2y3 < DGFixedPointMath.Epsilon) return INCOMPLETE;
			...
			else
			{
				...
				if (y2y3 < DGFixedPointMath.Epsilon)
				{
			...
			dx *= dx;
			dy = yp - yc;
			if (dx + dy * dy - rsqr <= DGFixedPointMath.Epsilon) return INSIDE;
