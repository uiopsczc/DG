...
namespace DG
{
	...
	/// </summary>
	public class DGEarClippingTriangulator
	{
		...
		public List<short> computeTriangles(List<DGFixedPoint> vertices)
		{
			return computeTriangles(vertices.ToArray(), 0, vertices.Count);
		}

		/** @see #computeTriangles(float[], int, int) */
		public List<short> computeTriangles(DGFixedPoint[] vertices)
		{
		 ...
		 *         method. */
		public List<short> computeTriangles(DGFixedPoint[] vertices, int offset, int count)
		{
			...
			if (DGGeometryUtils.isClockwise(vertices, offset, count))
			{
		...
		private int classifyVertex(int index)
		{
			...
			DGFixedPoint[] vertices = this.vertices;
		...
		private bool isEarTip(int earTipIndex)
		{
			...
			DGFixedPoint[] vertices = this.vertices;
			DGFixedPoint p1x = vertices[p1], p1y = vertices[p1 + 1];
			DGFixedPoint p2x = vertices[p2], p2y = vertices[p2 + 1];
			DGFixedPoint p3x = vertices[p3], p3y = vertices[p3 + 1];
			...
			for (int i = nextIndex(nextIndexTmp); i != previousIndexTmp; i = nextIndex(i))
			{
				...
				{
					int v = indices[i] * 2;
					DGFixedPoint vx = vertices[v];
					DGFixedPoint vy = vertices[v + 1];
		...
		private static int computeSpannedAreaSign(DGFixedPoint p1x, DGFixedPoint p1y, DGFixedPoint p2x, DGFixedPoint p2y,
			DGFixedPoint p3x, DGFixedPoint p3y)
		{
			DGFixedPoint area = p1x * (p3y - p2y);
			area += p2x * (p1y - p3y);
			area += p3x * (p2y - p1y);
			return DGFixedPointMath.Sign(area);
		}
	}
}
