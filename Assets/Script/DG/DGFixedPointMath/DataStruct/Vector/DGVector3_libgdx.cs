...
namespace DG
{
	...
	public partial struct DGVector3
	{
		...
		{
			DGFixedPoint cosPolar = DGFixedPointMath.Cos(polarAngle);
			DGFixedPoint sinPolar = DGFixedPointMath.Sin(polarAngle);

			DGFixedPoint cosAzim = DGFixedPointMath.Cos(azimuthalAngle);
			DGFixedPoint sinAzim = DGFixedPointMath.Sin(azimuthalAngle);
		...
		{
			return DGFixedPointMath.Sqrt(x * x + y * y + z * z);
		}
			...
			DGFixedPoint z2)
		{
			...
			return DGFixedPointMath.Sqrt(a * a + b * b + c * c);
		}
		...
		public DGVector3 nor()
		{
			...
			return this.scl((DGFixedPoint)1f / DGFixedPointMath.Sqrt(len2));
		}
		...
		public bool isOnLine(DGVector3 other)
		{
			return len2(y * other.z - z * other.y, z * other.x - x * other.z,
					   x * other.y - y * other.x) <= DGFixedPointMath.Epsilon;
		}
		...
		public DGVector3 slerp(DGVector3 target, DGFixedPoint alpha)
		{
			...
			dot = DGFixedPointMath.Clamp(dot, (DGFixedPoint)(-1.0f), (DGFixedPoint)1.0f);

			// Acos(dot) returns the angle between start and end,
			// And multiplying that by percent returns the angle between
			// start and the final result.
			DGFixedPoint theta = DGFixedPointMath.Acos(dot) * alpha;
			DGVector3 relativeVec = target - this * dot;
			relativeVec.Normalize();

			// Orthonormal basis
			// The final result.
			return (this * DGFixedPointMath.Cos(theta)) + (relativeVec * DGFixedPointMath.Sin(theta));
		}
		...
		public DGVector3 limit2(DGFixedPoint limit2)
		{
			...
			if (len2 > limit2)
			{
				scl(DGFixedPointMath.Sqrt(limit2 / len2));
			}
		...
		public bool epsilonEquals(DGVector3 other, DGFixedPoint epsilon)
		{
			...
			if (DGFixedPointMath.Abs(other.z - z) > epsilon) return false;
			return true;
		}
		...
		public bool epsilonEquals(DGFixedPoint x, DGFixedPoint y, DGFixedPoint z, DGFixedPoint epsilon)
		{
			...
			if (DGFixedPointMath.Abs(z - this.z) > epsilon) return false;
			return true;
		}
		...
		{
			return epsilonEquals(x, y, z, DGFixedPointMath.Epsilon);
		}
