...
namespace DG
{
	public partial struct DGVector2
	{
		...
		{
			return DGFixedPointMath.Sqrt(x * x + y * y);
		}
		...
		public static DGFixedPoint dst(DGFixedPoint x1, DGFixedPoint y1, DGFixedPoint x2, DGFixedPoint y2)
		{
			...
			return DGFixedPointMath.Sqrt(x_d * x_d + y_d * y_d);
		}
		...
		public DGVector2 limit2(DGFixedPoint limit2)
		{
			...
			if (len2 > limit2)
			{
				return scl(DGFixedPointMath.Sqrt(limit2 / len2));
			}
		...
		public DGVector2 clamp(DGFixedPoint min, DGFixedPoint max)
		{
			...
			if (len2 > max2) return scl(DGFixedPointMath.Sqrt(max2 / len2));
			DGFixedPoint min2 = min * min;
			if (len2 < min2) return scl(DGFixedPointMath.Sqrt(min2 / len2));
			return this;
		}
		...
		public DGVector2 setLength2(DGFixedPoint len2)
		{
			...
			return (oldLen2 == (DGFixedPoint)0 || oldLen2 == len2) ? this : scl(DGFixedPointMath.Sqrt(len2 / oldLen2));
		}
		...
		{
			DGFixedPoint angle = DGFixedPointMath.Atan2(y, x) * DGFixedPointMath.Rad2Deg;
		...
		{
			return DGFixedPointMath.Atan2(crs(reference), dot(reference)) * DGFixedPointMath.Rad2Deg;
		}
		...
		{
			DGFixedPoint angle = DGFixedPointMath.Atan2(reference.crs(this), reference.dot(this)) * DGFixedPointMath.Rad2Deg;
			if (angle < (DGFixedPoint)0) angle += (DGFixedPoint)360;
			return angle;
		}
		...
		{
			return DGFixedPointMath.Atan2(y, x);
		}
		...
		{
			return DGFixedPointMath.Atan2(reference.crs(this), reference.dot(this));
		}
		...
		{
			return setAngleRad(degrees * DGFixedPointMath.Deg2Rad);
		}
		...
		{
			return rotateRad(degrees * DGFixedPointMath.Deg2Rad);
		}
		...
		{
			DGFixedPoint cos = DGFixedPointMath.Cos(radians);
			DGFixedPoint sin = DGFixedPointMath.Sin(radians);
		...
		{
			if (DGFixedPointMath.Abs(other.x - x) > epsilon) return false;
			if (DGFixedPointMath.Abs(other.y - y) > epsilon) return false;
			return true;
		}
		...
		{
			if (DGFixedPointMath.Abs(x - this.x) > epsilon) return false;
			if (DGFixedPointMath.Abs(y - this.y) > epsilon) return false;
			return true;
		}
		...
		{
			return epsilonEquals(other, DGFixedPointMath.Epsilon);
		}
		...
		{
			return epsilonEquals(x, y, DGFixedPointMath.Epsilon);
		}
		...
		{
			return DGFixedPointMath.Abs(len2() - (DGFixedPoint)1f) < margin;
		}
		...
		{
			return DGFixedPointMath.IsZero(x * other.y - y * other.x);
		}

		public bool isOnLine(DGVector2 other, DGFixedPoint epsilon)
		{
			return DGFixedPointMath.IsZero(x * other.y - y * other.x, epsilon);
		}
		...
		{
			return DGFixedPointMath.IsZero(dot(vector));
		}

		public bool isPerpendicular(DGVector2 vector, DGFixedPoint epsilon)
		{
			return DGFixedPointMath.IsZero(dot(vector), epsilon);
		}
