	...
	public partial struct DGVector3
	{
		...
		*************************************************************************************/
		...
		public static DGVector3 ClampMagnitude(DGVector3 vector, DGFixedPoint maxLength)
		{
			...
			DGFixedPoint mag = DGFixedPointMath.Sqrt(sqrMagnitude);
		...
		public static DGVector3 OrthoNormalVector(DGVector3 vec)
		{
			...
			if (DGFixedPointMath.Abs(vec.z) > DGFixedPointMath.HalfSqrt2)
			{
				a = vec.y * vec.y + vec.z * vec.z;
				k = (DGFixedPoint)1 / DGFixedPointMath.Sqrt(a);
				x = (DGFixedPoint)0;
				y = -vec.z * k;
				z = vec.y * k;
				return new DGVector3(x, y, z);
		...
		public static DGVector3 RotateTowards(DGVector3 current, DGVector3 target, DGFixedPoint maxRadiansDelta, DGFixedPoint maxMagnitudeDelta)
		{
			...
			if (len1 > DGFixedPointMath.Epsilon && len2 > DGFixedPointMath.Epsilon)
			{
				var from = current / len1;
				var to = target / len2;
				var cosom = Dot(from, to);
				if (cosom > (DGFixedPoint)1 - DGFixedPointMath.Epsilon)
					return MoveTowards(current, target, maxMagnitudeDelta);
				if (cosom < (DGFixedPoint)(-1) + DGFixedPointMath.Epsilon)
				{
					var axis = OrthoNormalVector(@from);
					var q = DGQuaternion.AngleAxis(maxRadiansDelta * DGFixedPointMath.Rad2Deg, axis);
					var rotated = q * @from;
					var delta = DGFixedPointMath.ClampedMove(len1, len2, maxMagnitudeDelta);
					rotated = rotated * delta;
					return rotated;
				}

				{
					var angle = DGFixedPointMath.Acos(cosom);
					var axis = Cross(@from, to);
					axis.Normalize();
					var q = DGQuaternion.AngleAxis(DGFixedPointMath.Min(maxRadiansDelta, angle) * DGFixedPointMath.Rad2Deg, axis);
					var rotated = q * @from;
					var delta = DGFixedPointMath.ClampedMove(len1, len2, maxMagnitudeDelta);
					rotated = rotated * delta;
					return rotated;
		...
		public static DGVector3 MoveTowards(DGVector3 current, DGVector3 target, DGFixedPoint maxDistanceDelta)
		{
			...
			if (sqrtDistance == DGFixedPoint.Zero ||
				...
				return target;
			DGFixedPoint distance = DGFixedPointMath.Sqrt(sqrtDistance);
			...
			DGFixedPoint smoothTime, DGFixedPoint maxSpeed, DGFixedPoint deltaTime)
		{
			...
			smoothTime = DGFixedPointMath.Max((DGFixedPoint)0.0001F, smoothTime);
			...
			{
				var mag = DGFixedPointMath.Sqrt(sqrmag);
		...
		public static DGVector3 Project(DGVector3 vector, DGVector3 onNormal)
		{
			...
			if (sqrMag < DGFixedPointMath.Epsilon)
				return zero;
		...
		public static DGFixedPoint Angle(DGVector3 fromAngle, DGVector3 toAngle)
		{
			...
			DGFixedPoint dot = DGFixedPointMath.Clamp(Dot(fromAngle, toAngle) / denominator, DGFixedPoint.NegativeOne, DGFixedPoint.One);
			return DGFixedPointMath.Acos(dot) * DGFixedPointMath.Rad2Deg;
		}
		...
		public static DGFixedPoint SignedAngle(DGVector3 from, DGVector3 to, DGVector3 axis)
		{
			...
			DGFixedPoint sign = (DGFixedPoint)DGFixedPointMath.Sign(axis.x * crossX + axis.y * crossY + axis.z * crossZ);
			return unsignedAngle * sign;
		}

		public static DGVector3 Min(DGVector3 value1, DGVector3 value2)
		{
			return new DGVector3(DGFixedPointMath.Min(value1.x, value2.x), DGFixedPointMath.Min(value1.y, value2.y),
				DGFixedPointMath.Min(value1.z, value2.z));
		}

		public static DGVector3 Max(DGVector3 value1, DGVector3 value2)
		{
			return new DGVector3(DGFixedPointMath.Max(value1.x, value2.x), DGFixedPointMath.Max(value1.y, value2.y),
				DGFixedPointMath.Max(value1.z, value2.z));
		}

		public static DGVector3 Abs(DGVector3 value)
		{
			return new DGVector3(DGFixedPointMath.Abs(value.x), DGFixedPointMath.Abs(value.y), DGFixedPointMath.Abs(value.z));
		}

		public static DGVector3 SmoothStep(DGVector3 v1, DGVector3 v2, DGFixedPoint amount)
		{
			amount = DGFixedPointMath.Clamp01(amount);
