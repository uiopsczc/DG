...
namespace DG
{
	public partial class DGIntersector
	{
		...
		public static int pointLineSide(DGFixedPoint linePoint1X, DGFixedPoint linePoint1Y, DGFixedPoint linePoint2X, DGFixedPoint linePoint2Y, DGFixedPoint pointX,
			DGFixedPoint pointY)
		{
			return DGFixedPointMath.Sign((linePoint2X - linePoint1X) * (pointY - linePoint1Y) - (linePoint2Y - linePoint1Y) * (pointX - linePoint1X));
		}
		...
		public static DGFixedPoint distanceLinePoint(DGFixedPoint startX, DGFixedPoint startY, DGFixedPoint endX, DGFixedPoint endY, DGFixedPoint pointX, DGFixedPoint pointY)
		{
			...
			return DGFixedPointMath.Abs((pointX - startX) * (endY - startY) - (pointY - startY) * (endX - startX)) / normalLength;
		}
		...
		public static bool intersectPlanes(DGPlane a, DGPlane b, DGPlane c, ref DGVector3 intersection)
		{
			...
			if (DGFixedPointMath.Abs(f) < DGFixedPointMath.Epsilon)
			{
				return false;
			}
		...
		public static bool intersectRayTriangle(DGRay ray, DGVector3 t1, DGVector3 t2, DGVector3 t3, ref DGVector3 intersection)
		{
			...
			if (DGFixedPointMath.IsZero(det))
			{
				p.set(t1, t2, t3);
			...
			if (intersection != DGVector3.Null)
			{
				intersection = t <= DGFixedPointMath.Epsilon ? ray.origin : ray.getEndPoint(t);
			}
		...
		public static bool intersectRaySphere(DGRay ray, DGVector3 center, DGFixedPoint radius, ref DGVector3 intersection)
		{
			...
			if (intersection != DGVector3.Null)
				intersection = ray.direction.scl(len - DGFixedPointMath.Sqrt(r2 - dst2)).add(ray.origin);
			return true;
		}
		...
		public static bool intersectRayBoundsFast(DGRay ray, DGVector3 center, DGVector3 dimensions)
		{
			...
			DGFixedPoint min = DGFixedPointMath.Max(DGFixedPointMath.Max(minx, miny), minz);
			DGFixedPoint max = DGFixedPointMath.Min(DGFixedPointMath.Min(maxx, maxy), maxz);
		...
		public static bool intersectBoundsPlaneFast(DGVector3 center, DGVector3 halfDimensions, DGVector3 normal, DGFixedPoint distance)
		{
			...
			DGFixedPoint radius = halfDimensions.x * DGFixedPointMath.Abs(normal.x) + halfDimensions.y * DGFixedPointMath.Abs(normal.y)
				+ halfDimensions.z * DGFixedPointMath.Abs(normal.z);

			// Compute distance of box center from plane
			DGFixedPoint s = normal.dot(center) - distance;

			// Intersection occurs when plane distance falls within [-r,+r] interval
			return DGFixedPointMath.Abs(s) <= radius;
		}
		...
		public static bool intersectRectangles(DGRectangle rectangle1, DGRectangle rectangle2, ref DGRectangle intersection)
		{
			if (rectangle1.overlaps(rectangle2))
			{
				...
				{
					intersection.x = DGFixedPointMath.Max(rectangle1.x, rectangle2.x);
					intersection.width = DGFixedPointMath.Min(rectangle1.x + rectangle1.width, rectangle2.x + rectangle2.width) - intersection.x;
					intersection.y = DGFixedPointMath.Max(rectangle1.y, rectangle2.y);
					intersection.height = DGFixedPointMath.Min(rectangle1.y + rectangle1.height, rectangle2.y + rectangle2.height) - intersection.y;
				}
				return true;
			}
			return false;
			...
			ref DGMinimumTranslationVector mtv, bool shapesShifted)
		{
			...
			for (int i = offset1; i < endA; i += 2)
			{
				...
				for (int v = offset1; v < endA; v += 2)
				{
					...
					minA = DGFixedPointMath.Min(minA, p);
					maxA = DGFixedPointMath.Max(maxA, p);
				}

				DGFixedPoint minB = DGFixedPointMath.MaxValue;
				DGFixedPoint maxB = -DGFixedPointMath.MaxValue;

				// project shape b on axis
				for (int v = offset2; v < endB; v += 2)
				{
					DGFixedPoint p = verts2[v] * axisX + verts2[v + 1] * axisY;
					minB = DGFixedPointMath.Min(minB, p);
					maxB = DGFixedPointMath.Max(maxB, p);
				}
				// There is a gap
				...
				else
				{
					if (!mtv.Equals(DGMinimumTranslationVector.Null))
					{
						DGFixedPoint o = DGFixedPointMath.Min(maxA, maxB) - DGFixedPointMath.Max(minA, minB);
						...
						{
							mins = DGFixedPointMath.Abs(minA - minB);
							maxs = DGFixedPointMath.Abs(maxA - maxB);
							o += DGFixedPointMath.Min(mins, maxs);
						}
