...
namespace DG
{
	public partial class DGBoundingBox
	{
		...
		{
			return this.set(min.set(DGFixedPointMath.Min(min.x, point.x), DGFixedPointMath.Min(min.y, point.y), DGFixedPointMath.Min(min.z, point.z)),
				max.set(DGFixedPointMath.Max(max.x, point.x), DGFixedPointMath.Max(max.y, point.y), DGFixedPointMath.Max(max.z, point.z)));
		}
		...
		{
			return this.set(min.set(DGFixedPointMath.Min(min.x, a_bounds.min.x), DGFixedPointMath.Min(min.y, a_bounds.min.y), DGFixedPointMath.Min(min.z, a_bounds.min.z)),
				max.set(DGFixedPointMath.Max(max.x, a_bounds.max.x), DGFixedPointMath.Max(max.y, a_bounds.max.y), DGFixedPointMath.Max(max.z, a_bounds.max.z)));
		}
		...
		{
			return this.set(
				min.set(DGFixedPointMath.Min(min.x, center.x - radius), DGFixedPointMath.Min(min.y, center.y - radius), DGFixedPointMath.Min(min.z, center.z - radius)),
				max.set(DGFixedPointMath.Max(max.x, center.x + radius), DGFixedPointMath.Max(max.y, center.y + radius), DGFixedPointMath.Max(max.z, center.z + radius)));
		}
		...
		public bool intersects(DGBoundingBox b)
		{
			...
			DGFixedPoint lx = DGFixedPointMath.Abs(this.cnt.x - b.cnt.x);
			DGFixedPoint sumx = (this.dim.x / (DGFixedPoint)2.0f) + (b.dim.x / (DGFixedPoint)2.0f);

			DGFixedPoint ly = DGFixedPointMath.Abs(this.cnt.y - b.cnt.y);
			DGFixedPoint sumy = (this.dim.y / (DGFixedPoint)2.0f) + (b.dim.y / (DGFixedPoint)2.0f);

			DGFixedPoint lz = DGFixedPointMath.Abs(this.cnt.z - b.cnt.z);
		...
		{
			return this.set(min.set(DGFixedPointMath.Min(min.x, x), DGFixedPointMath.Min(min.y, y), DGFixedPointMath.Min(min.z, z)),
				max.set(DGFixedPointMath.Max(max.x, x), DGFixedPointMath.Max(max.y, y), DGFixedPointMath.Max(max.z, z)));
		}
	}
}
