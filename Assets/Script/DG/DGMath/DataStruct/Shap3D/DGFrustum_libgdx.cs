...
namespace DG
{
	...
	{
		protected static DGVector3[] clipSpacePlanePoints =
		{
		new DGVector3(-1, -1, -1), new DGVector3(1, -1, -1),
		new DGVector3(1, 1, -1), new DGVector3(-1, 1, -1), // near clip
		new DGVector3(-1, -1, 1), new DGVector3(1, -1, 1), new DGVector3(1, 1, 1), new DGVector3(-1, 1, 1)
	}; // far clip
		...
		public DGVector3[] planePoints =
		{
		new DGVector3(), new DGVector3(), new DGVector3(), new DGVector3(), new DGVector3(), new DGVector3(),
		...
		* @return Whether the bounding box is in the frustum */
		public bool boundsInFrustum(DGBoundingBox bounds)
		{
		...
		* @return Whether the bounding box is in the frustum */
		...
		{
			return boundsInFrustum(center.x, center.y, center.z, dimensions.x / (DGFixedPoint)2,
				dimensions.y / (DGFixedPoint)2, dimensions.z / (DGFixedPoint)2);
		}

		/** Returns whether the given bounding box is in the frustum.
		* @return Whether the bounding box is in the frustum */
		public bool boundsInFrustum(DGFixedPoint x, DGFixedPoint y, DGFixedPoint z, DGFixedPoint halfWidth,
			DGFixedPoint halfHeight, DGFixedPoint halfDepth)
		{
			for (int i = 0, len2 = planes.Length; i < len2; i++)
			{
		...
		* @return Whether the oriented bounding box is in the frustum */
		public bool boundsInFrustum(DGOrientedBoundingBox obb)
		{
			for (int i = 0, len2 = planes.Length; i < len2; i++)
			{
