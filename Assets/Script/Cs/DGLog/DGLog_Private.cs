...
public static partial class DGLog
{
	...
	private static string _DecorateLog(string msg, bool? isStackTrace)
	{
		...
		if(_Log_Cfg.enableThreadId)
			_Decorate_Log_String_Builder.Append(string.Format("[ThreadId:{0}]", Thread.CurrentThread.ManagedThreadId));
		...
		if (needStackTrace)
		{
			...
			for (int i = 0; i < stackTrace.FrameCount; i++)
			{
				...
				_Decorate_Log_String_Builder.Append(string.Format("\n	{0}:{1}()(at {2}:{3})", methodBase.DeclaringType.FullName, methodBase.Name, stackFrame.GetFileName(), stackFrame.GetFileLineNumber()));
			}
		}
