...
public class DGFixedPointTest
{
	...
	public void LongToFix64AndBack()
	{
		var sources = new[]
		{
			long.MinValue, int.MinValue - 1L, int.MinValue, -1L, 0L, 1L, int.MaxValue, int.MaxValue + 1L, long.MaxValue
		};
	...
	public void Addition()
	{
		...
		var expecteds = new[]
		{
			DGFixedPoint.MinValue, DGFixedPoint.One, (DGFixedPoint) (-1.5m), (DGFixedPoint) (-1), DGFixedPoint.MaxValue
		};
...
//					Assert.Throws<DivideByZeroException>(() => Ignore(f1 / f2));
		...
		Console.WriteLine("Max error: {0} ({1} times precision)", deltas.Max(), deltas.Max() / DGFixedPoint.Precision);
...
//				Assert.Throws<ArgumentOutOfRangeException>(() => DGFixedPoint.Acos(b));
		...
		Console.WriteLine("Max error: {0} ({1} times precision)", deltas.Max(), deltas.Max() / DGFixedPoint.Precision);
		Console.WriteLine("Average precision: {0} ({1} times precision)", deltas.Average(), deltas.Average() / DGFixedPoint.Precision);
	}
