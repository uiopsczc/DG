...
/// </summary>
public partial class DGConvexHull
{
	private List<int> quicksortStack = new List<int>();
	private DGFixedPoint[] sortedPoints;
	private List<DGFixedPoint> hull = new List<DGFixedPoint>();
	private List<int> indices = new List<int>();
	private List<short> originalIndices = new List<short>();

	/** @see #computePolygon(float[], int, int, boolean) */
	public List<DGFixedPoint> computePolygon(List<DGFixedPoint> points, bool sorted)
	{
		return computePolygon(points.ToArray(), 0, points.Count, sorted);
	}

	/** @see #computePolygon(float[], int, int, boolean) */
	public List<DGFixedPoint> computePolygon(DGFixedPoint[] polygon, bool sorted)
	{
	...
	public List<DGFixedPoint> computePolygon(DGFixedPoint[] points, int offset, int count, bool sorted)
	{
		...
		List<DGFixedPoint> hull = this.hull;
		hull.Clear();

		// Lower hull.
		for (int i = offset; i < end; i += 2)
		{
			DGFixedPoint x = points[i];
			DGFixedPoint y = points[i + 1];
			while (hull.Count >= 4 && ccw(x, y) <= (DGFixedPoint) 0)
			{
//				hull.Count -= 2;
		...
		for (int i = end - 4, t = hull.Count + 2; i >= offset; i -= 2)
		{
			DGFixedPoint x = points[i];
			DGFixedPoint y = points[i + 1];
			while (hull.Count >= t && ccw(x, y) <= (DGFixedPoint) 0)
			{
				//hull.size -= 2;
				hull.RemoveAt(hull.Count - 1);
				hull.RemoveAt(hull.Count - 1);
	...
	public List<int> computeIndices(List<DGFixedPoint> points, bool sorted, bool yDown)
	{
		return computeIndices(points.ToArray(), 0, points.Count, sorted, yDown);
	}

	/** @see #computeIndices(float[], int, int, boolean, boolean) */
	public List<int> computeIndices(DGFixedPoint[] polygon, bool sorted, bool yDown)
	{
	 ...
	 * points. */
	public List<int> computeIndices(DGFixedPoint[] points, int offset, int count, bool sorted, bool yDown)
	{
	...
	private DGFixedPoint ccw(DGFixedPoint p3x, DGFixedPoint p3y)
	{
		List<DGFixedPoint> hull = this.hull;
		int size = hull.Count;
		DGFixedPoint p1x = hull[size - 4];
		DGFixedPoint p1y = hull[size - 3];
		DGFixedPoint p2x = hull[size - 2];
		DGFixedPoint p2y = hull[size - 1];
	...
	private int quicksortPartition(DGFixedPoint[] values, int lower, int upper)
	{
		DGFixedPoint x = values[lower];
		DGFixedPoint y = values[lower + 1];
		int up = upper;
		int down = lower;
		DGFixedPoint temp;
		while (down < up)
	 ...
	 * @param count Number of indices, must be even. */
	private void sortWithIndices(DGFixedPoint[] values, int count, bool yDown)
	{
	...
	private int quicksortPartitionWithIndices(DGFixedPoint[] values, int lower, int upper, bool yDown,
