...
public static partial class DGGeometryUtils
{
	...
	{
		return barycentric.x >= (FP) 0 && barycentric.y >= (FP) 0 && barycentric.x + barycentric.y <= (FP) 1;
	}
	...
	{
		FP u = (FP) 1 - barycentric.x - barycentric.y;
	...
	{
		FP det = b * b - (FP) 4 * a * c;
		if (det < (FP) 0) throw new Exception("Float NaN");

		FP sqrtD = DGMath.Sqrt(det);
		FP invA = (FP) 1 / ((FP) 2 * a);
		FP r1 = (-b - sqrtD) * invA;
		FP r2 = (-b + sqrtD) * invA;
		...
		if (r1 > (FP) 0) return r1;
		if (r2 > (FP) 0) return r2;
	...
	public static FP triangleCircumradius(FP x1, FP y1, FP x2, FP y2, FP x3, FP y3)
	{
		...
		if (DGMath.Abs(y2 - y1) < DGMath.Epsilon)
		{
			m2 = -(x3 - x2) / (y3 - y2);
			mx2 = (x2 + x3) / (FP) 2;
			my2 = (y2 + y3) / (FP) 2;
		...
		else if (DGMath.Abs(y3 - y2) < DGMath.Epsilon)
		{
			m1 = -(x2 - x1) / (y2 - y1);
			mx1 = (x1 + x2) / (FP) 2;
			my1 = (y1 + y2) / (FP) 2;
		...
		FP dx = x1 - x, dy = y1 - y;
		return DGMath.Sqrt(dx * dx + dy * dy);
	}
	...
	public static FP triangleQuality(FP x1, FP y1, FP x2, FP y2, FP x3, FP y3)
	{
		...
		return DGMath.Sqrt(DGMath.Min(sqLength1, DGMath.Min(sqLength2, sqLength3))) /
		       triangleCircumradius(x1, y1, x2, y2, x3, y3);
	}

	public static FP triangleArea(FP x1, FP y1, FP x2, FP y2, FP x3, FP y3)
	{
		return DGMath.Abs((x1 - x3) * (y2 - y1) - (x1 - x2) * (y3 - y1)) * (FP) 0.5f;
	}

	public static FPVector2 quadrilateralCentroid(FP x1, FP y1, FP x2, FP y2, FP x3, FP y3, FP x4, FP y4,
		FPVector2 centroid)
	{
		FP avgX1 = (x1 + x2 + x3) / (FP) 3;
		FP avgY1 = (y1 + y2 + y3) / (FP) 3;
		FP avgX2 = (x1 + x4 + x3) / (FP) 3;
		FP avgY2 = (y1 + y4 + y3) / (FP) 3;
		centroid.x = avgX1 - (avgX1 - avgX2) / (FP) 2;
		centroid.y = avgY1 - (avgY1 - avgY2) / (FP) 2;
		return centroid;
	}
	...
	public static FPVector2 polygonCentroid(FP[] polygon, int offset, int count, FPVector2 centroid)
	{
		...
		{
			area *= (FP) 0.5f;
			centroid.x = x / ((FP) 6 * area);
			centroid.y = y / ((FP) 6 * area);
		}
